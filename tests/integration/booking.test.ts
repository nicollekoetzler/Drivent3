...
describe("GET /booking", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 404 when user has no ticket", async () => {
      ...
      const response = await server.get("/booking").set("Authorization", `Bearer ${token}`).send(verifyTicket);
...
describe("POST /booking", () => {
  ...
  describe("when token is valid", () => {
    ...
    it("should respond with status 404 when user has no ticket", async () => {
      ...
      const response = await server.post("/booking").set("Authorization", `Bearer ${token}`).send(verifyTicket);
